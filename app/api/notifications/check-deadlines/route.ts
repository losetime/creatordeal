  ...
  try {
    ...
    for (const pref of preferences) {
      try {
        ...
        // Check deal content deadlines
        if (pref.email_deadline_7d || pref.email_deadline_3d || pref.email_deadline_1d || pref.email_deadline_today) {
            ...
            for (const deal of deals) {
              ...
              if (shouldSend && daysLeft >= 0) {
                await resend.emails.send({
                  from: `${profile?.full_name || "Creator"} via CreatorDeal <${process.env.RESEND_FROM_EMAIL}>`,
                  to: [profile.email],
                  subject,
                  html: `
        ...
        if (pref.email_payment_overdue) {
          const { data: invoices } = await supabase
            .from("invoices")
            .select("id, invoice_number, amount, currency, due_date, paid_at, deals(title, brand_id, brands(name, contact_email))")
            .eq("user_id", pref.user_id)
